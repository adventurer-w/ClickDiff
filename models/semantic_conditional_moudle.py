...
class Trainer(object):
    ...
    def prep_head_condition_mask(self, data, joint_idx=15):
        ...
        mask[:,:,2048:] = torch.zeros(data.shape[0], data.shape[1], 11264).to(data.device)
        return mask 
    ...
    def full_body_gen_cond_head_pose_sliding_window(self, input_data):
        ...
        with torch.no_grad():
            ...
            val_data_in = torch.cat([input_data[:,:,6205:8253],val_feature.cuda(),input_data[:,:,8253:18493]],dim=-1).cuda()
            
            mano = input_data[:,:,6205:8253]
...
def get_moudle(opt, run_demo=False):
    ...
    trainer = Trainer(
        ...
        pointnet_model,
        train_batch_size=opt.diffusion_batch_size, 
        train_lr=opt.diffusion_learning_rate, 
